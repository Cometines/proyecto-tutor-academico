...
// Variable que fungirá como indicador de rangos dependiendo la posición(0-59, 60-69, 70-79...)
...
void asignarPosicion(float promedio){
    ...
    for(size_t i = 0; i < 5; i++){// Ciclo for para ir recorriendo el vector rangos
        ...
        else if(i == 3){//En caso de que el promedio no se encuentre dentro de un rango, se actualizarán los rangos a comparar en la proxima iteración.
            r1 = r2+1;
            r2 = 100;
        }
        else{
...
void mostrarHistograma(int estudiantes/*Cantidad de estudiantes*/){
    ...
    for (size_t i = 0; i < 5; i++){
        ...
        while (j <= porcentaje){ //La cantidad de guiones que se generarán respecto al porcentaje.
            ...
            pausarMilisegundos(15); //"Dormimos" al sistema para crear una animación pequeña de generación del histograma.
...
void iniciarHistograma(int Estudiantes){
    ...
    for (size_t i = 0; i < Estudiantes; i++){//El bucle iterará "Estudiantes" veces
        asignarPosicion(promediosEstudiantes[i]);//Se llama a la funcion y le mandamos el valor de la variable extern promedioEstudiantes[posicion i]
    }
